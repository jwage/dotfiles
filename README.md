...
| `hypr-kinetic-scroll/` | Momentum (kinetic) scrolling for the touchpad in terminals, as a vendored + locally patched Hyprland plugin. Terminals draw their own grid and handle scroll events directly, so unlike GTK/Chromium/Qt apps they never implement a fling of their own; this fills that gap at the compositor. Built by its own `build.sh` -- re-run it after any Hyprland upgrade |
