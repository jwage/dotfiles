...
BarWidget {
  ...
  // Qt's 's'/'ss' outside single-quoted literals is the seconds field.
  readonly property bool showsSeconds: /s/.test(String(activeFormat).replace(/'[^']*'/g, ""))
  ...
  SystemClock {
    id: clock
    // Ticking every second is only worth the wakeups when the label
    // actually paints seconds, so the precision follows the format.
