...
Vendored fork
-------------
Forked from https://github.com/brenoperucchi/magic-mouse-gestures at commit
7210b2d ("Add local publish script for manual releases", 2026-01-15), which was
upstream master at the time. The copy is kept close to upstream so a future
`git diff` against it stays readable -- see this directory's README.md for the
full list of local changes and how to re-sync.
"""
...
# Local change: upstream defaults to 1, which fires browser Back on one-finger
# horizontal motion -- and a finger rests on the Magic Mouse surface whenever the
# mouse is moved. Two fingers matches macOS, takes deliberate intent, and leaves
# one-finger motion to magicmouse-scroll (see ../README.md for the split).
MIN_FINGERS = get_env_int('MIN_FINGERS', 2)                  # Min fingers for swipe
